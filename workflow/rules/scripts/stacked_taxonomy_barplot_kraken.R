...
all_plot <- data.frame(
  Sample_Number=rep(c(rownames(kraken_top25_abun)), each = ncol(kraken_top25_abun)),
...
ggplot(all_plot, aes(fill=Species, y=Abundance, x=Sample_Number)) +
  geom_bar(position="fill", stat="identity", colour="black")+
  scale_fill_manual(values = color)+
  theme(axis.text.x = element_text(angle = 45, vjust = 1,
                                   size = 12, hjust = 1))+
  labs(title="Kraken2 taxonomy plot of the top 25 most prevalent species")
